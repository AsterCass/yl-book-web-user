<template>
  ...
  <div class="login-card column">
    ...
    <div class="q-px-md">
      ...
      <q-input v-model="inputPassword" dense outlined tabindex="0"
        ...
        <template v-slot:prepend>
          <div class="row items-center">
            ...
            <div style="opacity: 0.8">{{ $t('login.new_password') }}</div>
          </div>
        </template>
        <template v-slot:append>
      ...
      <div class="q-mx-sm q-mt-xs q-mb-sm" style="opacity: .5; font-size: .72rem">
      ...
      <div class="row justify-center q-mt-md">
        <q-btn no-caps unelevated class="shadow-2 component-full-btn-grow full-width"
               ...
               @click="doReset">
          <div class="row items-center">
            <div class="q-mr-sm" style="font-size: 14px">{{ $t('login.reset_submit') }}</div>
      ...
      <div class="row justify-center q-my-md">
        <span class="cask-jump-link-in-text" style="font-size: .85rem"
              @click="emit('switch', 'login')">{{ $t('login.back_to_login') }}</span>
      </div>
...
import {portalResetPasswd, portalResetPasswdSendCode} from "@/api/portal-auth.js";
...
function doSendCode() {
  ...
  portalResetPasswdSendCode({email: inputMail.value}).then(res => {
    if (!res) {
      return
    }
